...
- **TC-120** ✨ — no error pages. 403/404/500 all render Spring's Whitelabel: no `@ControllerAdvice`, no `ErrorController`, no `templates/error/`. The common case is an ordinary role denial, so it looks like a crash. Do before the public demo.
...
- **Next:** TC-120 (error pages) + TC-121 (day-one admin bootstrap) → TC-115/TC-116 (CI image, AWS demo) → TC-110 → TC-111 → TC-112.
