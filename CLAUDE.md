...
- **TC-121** — **day-one admin bootstrap.** `BootstrapAdminSeeder` (`@Profile("prod & !demo")`) creates the first ADMIN from `SSTATION_BOOTSTRAP_ADMIN_USERNAME`/`_PASSWORD` **only when the `users` table is empty**. That account is flagged `must_change_password` (Flyway **V5**) and `MustChangePasswordFilter` confines it to `/change-password` until it sets its own. Also adds the in-app password change the app never had (`PasswordChangeService`, `/change-password`) and a shared `PasswordPolicy` (min 8) now enforced on **both** password-setting paths. See the **First login** section of [DEPLOY.md](DEPLOY.md), which also documents a verified break-glass SQL recovery.
- **TC-115** — **CI builds the container image and proves it runs.** `ci-next.yml` gains an `image` job (needs `check`) that builds with a `type=gha` layer cache, brings up the real `docker-compose.yml` via a `docker-compose.ci.yml` override, and runs [scripts/smoke-container.sh](sstation-next/scripts/smoke-container.sh) — the image's own HEALTHCHECK, `/actuator/health`, `/login`, a demo-admin sign-in, and that `admin_secret` is still rejected. On pushes to `main` it publishes `ghcr.io/<owner>/sstation-next:{sha,latest}` so a small host can pull rather than compile (see TC-116's retired EC2 notes for why RAM is the constraint).
- **TC-123** — **student CSV import** (`/admin/students/import`, `StudentCsvImportService`), the only user-visible Grails feature the rewrite had missed — found by TC-111's parity checklist. Matches the registrar column layout exactly (zero-indexed `0` acid, `2` firstname, `3` lastname, `4` status, `5` acBox, `6` classification, `8` acEmail — **`1` and `7` are deliberately unread**), skips the header row, and adds-or-updates by student ID. Two deviations from Grails on purpose: invalid rows are **reported with line numbers** instead of silently dropped, and **students cannot reach it** (the Grails controller's class-level `@Secured` included `ROLE_STUDENT`, letting any student overwrite the roster). Multipart is capped at 5 MB.
- **TC-110** — **the self-hosting runbook, and the primary deliverable of the whole rewrite.** [DEPLOY.md](DEPLOY.md) is now instructions AC IT can follow on their own infrastructure, not ours: prerequisites (JDK 21 or just Docker, Postgres 13+, optional SMTP relay), the **three deployment shapes** (Docker on a VM, plain `java -jar`, behind whatever reverse proxy they already run — no cloud-provider-specific main path), a prominent **"a single container is a valid deployment"** note (in-memory sessions are fine; `spring-session-jdbc` is a Vercel constraint only), first login (TC-121) kept prominent, **backups and upgrades** (where the data lives — the `sstation_pg` volume — `pg_dump`/`psql`, Flyway auto-migrates on boot, honest single-container restart downtime), every AC IT-supplied value marked 🏫 (TC-035's placeholder set), and the env-var set matching the Grails plan's with two documented differences (`SSTATION_MAIL_USER` vs `USERNAME`; no `SSTATION_SERVER_URL` — the app derives its URL from the request). **Writing it found a real bug:** password-reset links came out `http://…` behind a TLS-terminating proxy — Boot's `server.forward-headers-strategy` defaults to `NONE` and `ServletUriComponentsBuilder.fromCurrentContextPath()` reads the raw request scheme — fixed with `server.forward-headers-strategy: framework` in the `prod` profile (TC-110b), a focused test, and a live container verification.

### Still to do in the rewrite

**The goal, clarified 2026-09-07:** nobody is standing up a service that all of Austin College then uses. **AC IT hosts it themselves.** Everything below is either the instructions they follow, or a demo that exists to convince them to follow them.

- **TC-122** ☁️ — **public demo on Vercel** (decided 2026-09-07; supersedes the AWS plan in TC-116, which supersedes App Runner in TC-109). A showcase, not infrastructure: no real data, no uptime expectation. Vercel now runs OCI container images as Functions on Fluid compute, so the TC-113/TC-115 container work carries over — a hosting swap, not a re-architecture. **Three things are real work, not config:** sessions must move to `spring-session-jdbc` (in-memory Tomcat sessions today; Fluid compute may serve a request from any instance, so signed-in users get bounced to the login page at random — **a Vercel constraint, not an app deficiency**); `server.port` is hardcoded to `8080` and must honour `$PORT`; and a `vercel.json` `services` entrypoint pointing at the existing tested Dockerfile. On **Supabase free**: use the **Supavisor shared pooler in session mode** (free-tier direct connections are IPv6-only; transaction mode breaks JDBC prepared statements) and expect the project to **pause after 7 idle days** unless a daily Vercel Cron hits `/actuator/health`.
- **TC-111** 🟡 — **the E2E suite has landed; the sign-off has not.** `src/e2eTest` is a 31-test Playwright suite driving a real browser against the compose stack, wired into CI's `image` job, plus [docs/TC-111-parity-checklist.md](docs/TC-111-parity-checklist.md). The one regression it found — the missing student CSV import — **was closed by TC-123**. What remains is not code: a Service Station stakeholder clicking through, and **importing one real registrar CSV to confirm the column layout** (matched against the Grails source, never against an actual export).
- **TC-112** — decommission the Grails app, once TC-111 is signed off.
...
- **The Postgres path is covered locally *and* in CI, and a skipped test now fails the build (TC-118).** Three tests touch real PostgreSQL via Testcontainers (`postgres:16-alpine`): `DemoProfileIntegrationTest` (profiles `prod,demo`), `BootstrapAdminIntegrationTest` (bare `prod` + the TC-121 bootstrap vars) and `ProdProfileMigrationIntegrationTest` (bare `prod` on a **virgin** database — Flyway V1–V5 apply cleanly, `ddl-auto=validate` agrees, no seeder leaks into prod). They use separate containers on purpose; sharing one would make "virgin" false and would leave accounts behind for the bootstrap seeder, which only acts on an empty table. Two things make this work and are easy to break: `build.gradle.kts` sets docker-java's **`api.version` system property to 1.41** on the test JVM (docker-java defaults to v1.32; Docker Engine 29's `MinAPIVersion` is 1.40, so `/v1.32/info` returns HTTP 400, every Testcontainers strategy fails, and `disabledWithoutDocker = true` turns that into a **silent skip under a BUILD SUCCESSFUL**), and Testcontainers is pinned via **`extra["testcontainers.version"]`** rather than explicit coordinates, because Spring Boot's BOM otherwise drags core back to 1.19.8 behind whatever version you declare. `check` also depends on **`verifyNoSkippedTests`**, which parses the JUnit XML and fails if *any* test class reports `skipped > 0` — the suite is **152/152 with zero skips** on both a dev box and the runner, so a skip is always signal. If that task fires, read its message before "fixing" the test.
...
- **Email is log-only until a relay is configured — but not for the reason you'd guess.** `SSTATION_MAIL_HOST` defaults to empty so containers boot without SMTP. That does **not** mean no `JavaMailSender` exists: under `prod`, `spring.mail.host` binds to `${SSTATION_MAIL_HOST:}` and is therefore **present but empty**, and `@ConditionalOnProperty` counts present-but-empty as *set* — so Spring builds a mail sender pointed at nowhere. Two consequences, both fixed in TC-115 and both easy to reintroduce: `NotificationService` decides "log-only" by checking **the host string**, not whether the bean exists (checking the bean meant every approve/reject and password reset opened a doomed SMTP connection and swallowed the error), and `management.health.mail.enabled` defaults to **false**, because otherwise `MailHealthIndicator` fails to connect and drags all of `/actuator/health` to DOWN — which means the container's HEALTHCHECK never passes, `docker compose up` sits at "starting" forever, and any orchestrator considers the app dead. Set `SSTATION_MAIL_HEALTH_ENABLED=true` once SMTP is real. Approve/reject and password-reset mail still silently won't send until AC IT supplies a relay — **the app gives no error**. Don't debug that as a mail bug.
...
- **`password_expired` and `must_change_password` are different things — don't merge them.** `password_expired` maps to Spring Security's `credentialsExpired` and **fails authentication outright**; using it to mean "please pick a new password" locks the user out of an app whose only reset path needs SMTP that may not be configured. `must_change_password` (V5, TC-121) lets them in and then confines them to `/change-password` via `MustChangePasswordFilter`.
- **The must-change flag rides on the principal (`SstationUserDetails`), not a per-request query** — so it is a snapshot taken at login. That is why a successful change **ends the session** and returns to `/login?changed` instead of patching the principal; if you make the change flow stay signed in, the filter will bounce that user forever. `/logout`, `/login` and `/error` are on the filter's allowlist for the same class of reason — removing `/logout` traps the account.
- **`BootstrapAdminSeeder` is gated on the `users` table being empty, not on the username.** It therefore can never resurrect a deliberately deleted admin, and leaving `SSTATION_BOOTSTRAP_ADMIN_PASSWORD` set after the first boot is harmless. It is `@Profile("prod & !demo")` because under `prod,demo` the demo seeder owns account creation and two runners racing for an empty table would resolve by bean ordering.
- **The E2E suite (TC-111) is a separate source set and a separate task.** `src/e2eTest`, run with `./gradlew e2eTest` against an **already-running** stack (`docker compose -f docker-compose.yml -f docker-compose.ci.yml up -d`), after `./gradlew playwrightInstall` once. **`check` deliberately does not run it** — the unit gate stays hermetic and Docker-free. Point it elsewhere with `E2E_BASE_URL` (that is how it would be aimed at the TC-122 demo). Three traps cost real time when writing it, all now handled in `E2eTest` rather than per test:
