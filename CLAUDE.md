...
- **TC-111** 🟡 — **the E2E suite has landed; the sign-off has not.** `src/e2eTest` is a 26-test Playwright suite driving a real browser against the compose stack, wired into CI's `image` job, plus [docs/TC-111-parity-checklist.md](docs/TC-111-parity-checklist.md). What remains is not code: a Service Station stakeholder clicking through, and a decision on the regression the checklist found — **student CSV import was never ported** (Grails has `AcStudentController.upload()` → `StudentService.importStudents()`; `sstation-next` has no `MultipartFile` anywhere). Whether that blocks sign-off is the office's call, not a technical one.
