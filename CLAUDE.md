...
- `src/main/resources/` — `application.yml`, `templates/` (Thymeleaf; all pages decorate `templates/fragments/layout.html`), `db/migration/` **`V1__initial_schema.sql` → `V6__spring_session.sql`** (V2 audit log, V3 event signups, V4 reset tokens, V5 forced password change, V6 spring-session tables for the TC-122 Vercel demo). Frontend assets are vendored via WebJars (no `static/` blobs), served at `/webjars/**`.
...
- **TC-111 / TC-112** — E2E acceptance suite + stakeholder sign-off, then decommission the Grails app.
