...
- **TC-110** 📝 — **the self-hosting runbook, and the primary deliverable of the whole rewrite.** What AC IT runs on their own infrastructure: Docker or plain `java -jar`, Postgres, env vars, first login (TC-121), backups, upgrades. Must work on a plain VM behind whatever proxy they already run — no cloud-provider-specific main path. Note that a **single container is a valid deployment** for an office this size, so `spring-session-jdbc` is optional for them.
