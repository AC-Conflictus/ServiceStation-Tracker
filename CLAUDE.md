...
- **TC-118** ⚠️ — the Testcontainers Postgres test **skips silently** on a Docker 29.x dev box (see gotchas below). CI does run it. Do this before the AWS demo.
...
- **Next:** TC-118 (make the Postgres test's skip loud) → TC-115/TC-116 (CI image, AWS demo) → TC-110 → TC-111 → TC-112.
