...
### TC-116 ☁️ AWS public demo — EC2 t3.micro + RDS Postgres — ⏸️ **superseded by [TC-122](#tc-122-) (Vercel), 2026-09-07**
- **Status:** ⏸️ **Not doing — superseded 2026-09-07.** The team chose **Vercel** instead; see **[TC-122](#tc-122-)**. Kept on the board as a documented alternate (same as [TC-109](#tc-109-)) because the topology, security groups and cost notes below stay accurate if AWS is ever revisited. **Do not start this card.**
...
- **Acceptance criteria:**
  ...
  - Smoke test equivalent to [scripts/smoke-container.sh](sstation-next/scripts/smoke-container.sh) run against the deployed URL: dashboard KPIs non-zero, student login lands on a populated dashboard, one report renders.
...
- **Why:** `DemoProfileIntegrationTest` (TC-114) was the **only** test touching real PostgreSQL — Testcontainers a `postgres:16-alpine`, run Flyway, seed, log in. Annotated `@Testcontainers(disabledWithoutDocker = true)`, so when Testcontainers could not reach Docker it reported **SKIPPED while `./gradlew check` still said BUILD SUCCESSFUL**.
- **Why it matters:** Flyway `V1`–`V4` are hand-written in the "Postgres/H2-common subset" and `ddl-auto=validate` runs on every boot. If a migration or entity mapping is subtly H2-only, the Postgres test is the only thing standing between that and RDS. CI did catch it — but a developer running `./gradlew check` locally got a **green build that proved nothing about Postgres**. The failure mode was a false sense of safety, not (as first assessed) an absence of coverage.
- **Root cause (found 2026-09-04):** Testcontainers' shaded docker-java negotiates Docker Engine API **v1.32**. Docker Engine 29 raised its `MinAPIVersion` to **1.40**, so `/v1.32/info` returns **HTTP 400** and every client-provider strategy fails. Confirmed directly against the socket: `/v1.32/info` → 400, `/v1.40/info` and above → 200.
  - **The earlier dead end is explained too.** Bumping Testcontainers to 1.21.3 "did not help" because *the bump never took effect*: Spring Boot 3.3.5's BOM manages `testcontainers-bom` at 1.19.8, and a BOM beats an explicit version on a transitively-resolved artifact — so `junit-jupiter:1.20.3` resolved core as **`testcontainers:1.20.3 -> 1.19.8`**. Docker strategy detection lives in core. The build had been running a mixed-version classpath the whole time.
- **Acceptance criteria:**
  - ✅ **Root-cause the docker-java/Docker-29 mismatch.** Done — see above. Fixed by setting docker-java's `api.version` system property on the test JVM to **1.41**, chosen for range rather than recency: it clears Docker 29's floor of 1.40 and is still supported back to Docker 20.10.
  - ✅ **Make the skip loud.** `check` now depends on a `verifyNoSkippedTests` task that parses the JUnit XML and **fails the build if any test class reports `skipped > 0`**, naming the classes and pointing at the Docker API cause. No allowlist to maintain — the suite is 122/122 with zero skips on both a dev box and the runner, so a skip is always signal. Verified in both directions: green today, and forcing `api.version` back to 1.32 fails the build with the expected message.
  - ✅ **CI genuinely runs the Postgres path** — confirmed 2026-09-02 from run 33644576602's test XML (`tests=2 skipped=0`). Re-verify if the runner image changes.
  - ✅ **Second Postgres-backed test for the bare-`prod` day-one path.** [ProdProfileMigrationIntegrationTest](sstation-next/src/test/java/edu/austincollege/sstation/config/ProdProfileMigrationIntegrationTest.java) boots `prod` alone against a **virgin** container (deliberately not shared with the demo test — sharing would make "virgin" false) and asserts Flyway applies V1–V4 with nothing pending or failed, `ddl-auto=validate` agrees, no seeder leaked into production, and the dev credentials cannot authenticate.
- **Notes / files touched:** [build.gradle.kts](sstation-next/build.gradle.kts) (api.version, BOM pin, `verifyNoSkippedTests`), [ProdProfileMigrationIntegrationTest.java](sstation-next/src/test/java/edu/austincollege/sstation/config/ProdProfileMigrationIntegrationTest.java) (new). `DemoProfileIntegrationTest` and `ci-next.yml` were left unchanged — neither needed a fix.
- **Follow-on:** the bare-`prod` test proved a real gap rather than closing one — an empty user table with no way to create the first admin. Carded as **TC-121**.
- **Estimate:** S–M. *(Actual: S.)*
...
### TC-120 ✨ Error pages — 403 / 404 / 500 all rendered the framework default ✅ landed 2026-09-05
- **Status:** ✅ **Done.** Landed in four slices on `feat/tc-120-error-pages`: (a) the templates, (b) the `@ControllerAdvice`, (c) JSON content negotiation, (d) a fix from the live pass. Suite went 122 → **132 tests, 0 skipped**.
- **Why:** There was **no `@ControllerAdvice`, no `ErrorController`, and no `templates/error/`** anywhere in `sstation-next`. Every failure path fell through to Spring Boot's default error page: bare white page, no navbar, no link back, nothing identifying the application. The Grails app being replaced rendered errors inside its own layout, so this was a **regression against the thing we claim parity with** (TC-111).
- **Why it mattered:** the most likely 403 is an ordinary one — a student following a bookmarked `/admin/**` URL, or a moderator on an ADMIN-only route. Those are expected states of a role-based app, not crashes, and they looked like the app had fallen over. On the public AWS demo (**TC-116**) it is the first impression a reviewer gets.
- **Acceptance criteria:**
  - ✅ `templates/error/{403,404,500}.html` + catch-all `templates/error.html`, each decorating `fragments/layout.html`. Boot's `DefaultErrorViewResolver` selects by status; the catch-all covers 405/409/503 so nothing falls through.
  - ✅ The 403 reads as a permissions message rather than a failure — it names the signed-in account and offers switch-account / sign-out.
  - ✅ `GlobalErrorAdvice` maps uncaught exceptions to the 500 view and logs method + path at ERROR with the throwable. The page shows no detail deliberately, so the log is the only record and has to be searchable by the address the page tells the user to report.
  - ✅ **The JSON endpoint stays JSON**, and the real bug here was worse than the card guessed — see below.
  - ✅ `server.error.include-message` / `include-stacktrace` confirmed at their `never` defaults; asserted by `errorPagesDoNotLeakInternals`.
  - ✅ Tests — **but not MockMvc**, which the card wrongly specified. MockMvc stops at the status code and never runs the ERROR dispatch, so it can prove a request was rejected but not what the user then looks at, which is the whole point. `ErrorPageIntegrationTest` drives a real server on a random port over the JDK HTTP client: 10 tests covering 403/404/500/405 rendering inside the app shell, the ERROR log, no internals leaked, JSON staying JSON, and anonymous users still reaching sign-in.
- **What the card underestimated — the JSON endpoint was already broken, not merely at risk.** The concern was that a denial might start returning HTML. In fact an **expired session** was worse: Spring Security redirected the `fetch` to `/login` with a 302, fetch followed it, and the JS got the sign-in page back with **status 200**. `r.ok` passed, so it went into `r.json()` and died on `Unexpected token '<'` — a parse error three frames from the real problem, which was simply "you are signed out". Reproduced first, then fixed: `JsonApiRequestMatcher` is now the single definition of "this caller wants JSON", JSON callers get 401/403 with a JSON body, browsers are untouched, and the hours-page fetch sets `Accept` explicitly rather than hoping `*/*` is guessed correctly.
- **Two traps worth remembering (both caught by tests going red, not by review):**
  - `defaultAccessDeniedHandlerFor` / `defaultAuthenticationEntryPointFor` **ignore the matcher when they hold exactly one mapping** and apply the handler to every request — so registering "JSON callers get JSON" silently made *browsers* get JSON 403s. The delegation is now spelled out explicitly.
  - A `@ExceptionHandler(Exception.class)` swallows Spring MVC's `ErrorResponse` family, turning every 404 into a 500, and swallows Spring Security's exceptions, breaking both the anonymous sign-in redirect and the 403 render. Both are rethrown by identity, which leaves them unresolved without a spurious warning.
- **Notes / files touched:** [templates/error/](sstation-next/src/main/resources/templates/error/), [templates/error.html](sstation-next/src/main/resources/templates/error.html), [GlobalErrorAdvice.java](sstation-next/src/main/java/edu/austincollege/sstation/web/GlobalErrorAdvice.java) (new), [JsonApiRequestMatcher.java](sstation-next/src/main/java/edu/austincollege/sstation/security/JsonApiRequestMatcher.java) (new), [SecurityConfig.java](sstation-next/src/main/java/edu/austincollege/sstation/security/SecurityConfig.java), [hours/list.html](sstation-next/src/main/resources/templates/hours/list.html), [ErrorPageIntegrationTest.java](sstation-next/src/test/java/edu/austincollege/sstation/web/ErrorPageIntegrationTest.java) (new).
- **Verified live**, not just green: 403/404/405 rendered in a browser session with the navbar and footer intact, and the three JSON paths checked against a running app (anonymous JSON → 401 JSON, anonymous browser → still 302 to `/login`, moderator JSON → 403 JSON). The live pass also caught the string "Whitelabel" shipping to the browser inside an explanatory HTML comment of mine — fixed in (d), with the assertion tightened to the word so it would have caught it.
- **Depends on:** TC-107 ✅.
- **Estimate:** S. *(Actual: S–M — the JSON negotiation was most of it.)*

### TC-121 🔒 🏫 Day-one admin bootstrap — a bare `prod` boot had no account to log in with ✅ landed 2026-09-05
- **Status:** ✅ **Done.** Landed in five slices on `feat/tc-121-admin-bootstrap`. Suite went 132 → **152 tests, 0 skipped**. The blocking gap in the AC IT handoff is closed: a virgin production database now yields exactly one administrator who can sign in, and is made to replace the credential immediately.
- **Why:** A bare `prod` boot **never seeds** — correct and deliberate (`DevDataSeeder` is `dev`, `DemoAccountSeeder`/`DemoDataSeeder` are `demo`), and asserted by `ProdProfileMigrationIntegrationTest.bareProdSeedsNothing`. But **nothing created the first admin**: no CLI, no first-run signup, no bootstrap flag, and [DEPLOY.md](DEPLOY.md) documented the database and SMTP variables while never mentioning the first login. AC IT would have been handed an application they could not get into.
- **Why it was invisible:** the `demo` profile papers over it. Every environment anyone had actually run — local `dev`, Docker, the planned AWS showcase — seeds accounts, so the bare-`prod` path had never once been exercised by a human. It took writing a test for it (TC-118d) to see it at all.
- **What shipped, and why it grew past "create a user":**
  - **(a) An in-app password change.** There wasn't one. The *only* way to change a password was the emailed reset link, and `SSTATION_MAIL_HOST` defaults to empty — so on a fresh deployment no mail is sent and that path does not work. Whatever password an account was created with was the password it kept, permanently. Without this, option (a) below would just have moved the problem.
  - **(b) A forced change**, via a new `must_change_password` column (Flyway **V5**) and `MustChangePasswordFilter`.
  - **(c) The bootstrap runner** itself, `@Profile("prod & !demo")`.
  - **(d)** A Postgres-backed test of the whole path; **(e)** DEPLOY.md, `.env.example`, CLAUDE.md, README-NEXT.
- **Acceptance criteria:**
  - ✅ **(a) One-shot bootstrap runner — chosen, as recommended.** `BootstrapAdminSeeder` creates one ADMIN from `SSTATION_BOOTSTRAP_ADMIN_USERNAME` (default `admin`) / `SSTATION_BOOTSTRAP_ADMIN_PASSWORD`, **only when the `users` table is empty**, logging loudly at WARN. Gating on the empty table rather than the username means it can never resurrect a deliberately deleted admin, and leaving the variables set is harmless from the second boot on.
  - ✅ **(b) documented as the break-glass path** — see below; it is the answer for "already running, locked out", which the runner deliberately does not handle.
  - ⛔ **(c) CLI flag** — not pursued. Redundant once (a) exists.
  - ✅ **Forced password change**, not merely documented rotation. The bootstrap password arrives in an environment variable that stays readable in the deployment's own config, so it is single-use by design. **Re-entering it is refused** — without that the forced change is satisfiable by retyping the value we are trying to retire.
  - ✅ **DEPLOY.md has a "First login" section**, so TC-110 is no longer blocked on this.
  - ✅ **Postgres-backed test** alongside `ProdProfileMigrationIntegrationTest`: exactly one ADMIN, who authenticates, who is sent to `/change-password`, and who is not duplicated on restart.
- **Three design decisions worth keeping:**
  - **A new column, not the existing `password_expired`.** That one is already mapped to Spring Security's `credentialsExpired`, which **fails authentication outright** — it would have locked AC IT out on their first login rather than prompting them, with no working reset to recover through.
  - **`prod & !demo`, not `prod`.** Under `prod,demo` the demo seeder owns account creation; two runners racing for an empty table would make the winner depend on bean ordering.
  - **Missing password warns, weak password fails.** A bare `prod` boot with no accounts is a legitimate state an operator may intend, so it must not be fatal — but it must not be silent either, because the symptom (a sign-in form that rejects everything) looks nothing like the cause. A *weak* password is different: the operator clearly meant to bootstrap an admin, and quietly creating a weak one on an internet-facing host is the worse outcome.
- **Found and fixed on the way:** `PasswordResetService.reset()` had **no server-side length check at all** — the only rule anywhere was `minlength="6"` on the form, which any client that isn't a cooperating browser ignores. Both password-setting paths now go through a shared `PasswordPolicy` (minimum 8). One existing test used a 5-character password and was updated.
- **Verified live against real PostgreSQL 16**, not just green: bootstrap WARN on boot → sign in with the environment credential → every page redirects to `/change-password` → reuse refused, short password refused → change succeeds → session ends → old credential rejected → new credential reaches `/admin`. **The break-glass SQL in DEPLOY.md was executed too** — hash generated with the documented one-liner, both statements applied, resulting account signed in and was correctly forced to change.
- **Notes / files touched:** [BootstrapAdminSeeder.java](sstation-next/src/main/java/edu/austincollege/sstation/config/BootstrapAdminSeeder.java), [MustChangePasswordFilter.java](sstation-next/src/main/java/edu/austincollege/sstation/security/MustChangePasswordFilter.java), [SstationUserDetails.java](sstation-next/src/main/java/edu/austincollege/sstation/security/SstationUserDetails.java), [PasswordChangeService.java](sstation-next/src/main/java/edu/austincollege/sstation/service/PasswordChangeService.java), [PasswordPolicy.java](sstation-next/src/main/java/edu/austincollege/sstation/service/PasswordPolicy.java), [ChangePasswordController.java](sstation-next/src/main/java/edu/austincollege/sstation/web/ChangePasswordController.java) (all new), `V5__must_change_password.sql`, `User`, `CustomUserDetailsService`, `SecurityConfig`, `PasswordResetService`, `change-password.html`, `login.html`, [DEPLOY.md](DEPLOY.md), `.env.example`.
- **Estimate:** S–M. *(Actual: M — the missing password-change flow was not in the original scoping.)*

### TC-123 ✨ Student CSV import — the parity gap TC-111 found ✅ landed 2026-09-13
- **Status:** ✅ **Done.** `/admin/students/import`, reachable from the students list. Built on `feat/tc-123-student-csv-import` after the TC-111 parity checklist identified it as the **only** user-visible Grails feature the rewrite had missed.
- **Why:** The Grails app bulk-imports students from a registrar CSV (`AcStudentController.upload()` → `StudentService.importStudents()`, with `uploadPage`/`uploadSuccess` views). `sstation-next` had no `MultipartFile` anywhere — students could only be created one at a time, which does not survive a term rollover if the office loads a roster.
- **What was matched, deliberately:** the **column layout** (zero-indexed `0` acid, `1` ignored, `2` firstname, `3` lastname, `4` status, `5` acBox, `6` classification, `7` ignored, `8` acEmail), the header row being skipped, add-or-update **by student ID**, and an unrecognised classification becoming `OTHER` rather than failing the row. Matching the layout mattered more than matching the code: a column-order mismatch imports plausible garbage instead of failing, so it is pinned by test.
- **Two deliberate deviations, both improvements:**
  - **Invalid rows are reported, not dropped.** Grails counted only successes — 500 students with 12 bad rows reported "488 added" and said nothing else. Skipped rows now come back with line number and reason, and the good rows still import.
  - **Students can no longer do it.** The Grails `@Secured` on `AcStudentController` was class-level and listed `ROLE_STUDENT`, so any signed-in student could overwrite the entire roster. That is an accident of a grab-bag controller (it also hosts the student's own dashboard), not a decision. Now ADMIN + MODERATOR, matching `StudentAdminController`. **A test guards this specifically** — it is exactly the kind of rule someone loosens later while "restoring parity".
- **Also:** multipart capped at 5 MB (~50k rows) rather than Boot's 1 MB default, whose failure mode is an exception page rather than a message.
- **Verified:** 20 new tests — 9 on parsing semantics (column offsets, header skip, re-import, quoted commas, short rows, unknown classification), 6 on the web layer (permissions, CSRF, empty file), 5 end-to-end through a real file picker, which is the one thing `MockMvc.multipart()` simulates rather than performs. `check` 171, E2E 31, green against the compose stack on Postgres.
- **⚠️ Residual risk for sign-off:** the layout was matched against the **Grails source, not a real registrar export** — nobody on this project has seen the actual file. [The parity checklist](docs/TC-111-parity-checklist.md) asks the office to import one real export and confirm the columns land correctly before signing off.
